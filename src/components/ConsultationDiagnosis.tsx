...
import {IConsultationDiagnosis} from "../types/diagnosis";
import {RiStethoscopeLine} from "react-icons/ri";
import {useFindUser} from "../hooks/api/user";
import {Accordion, AccordionButton, AccordionIcon, AccordionItem, AccordionPanel} from "@chakra-ui/react";

interface Props {
    consultationDiagnosis: IConsultationDiagnosis
}

const ConsultationDiagnosis: React.FC<Props> = ({consultationDiagnosis: {createdAt, consultationId, closedAt, diagnosis, doctorUserId}}) => {
                        ...
                        <AccordionPanel p={0}>
                            <div className='divide-y divide-slate-200'>
                                {diagnosis.map((diagnosis, i) => (
                                    <div key={i}>
                                        <p className='px-2 py-1.5 line-clamp-2'>{diagnosis}</p>
                                    </div>
                                ))}
                            </div>
                        </AccordionPanel>
