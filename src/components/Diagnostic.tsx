...
interface DiagnosticProps {
    diagnostic: IDiagnosis
    setNewDiagnostic: (diagnostic: IDiagnosis) => void
    editable: boolean
}

const Diagnostic: React.FC<DiagnosticProps> = ({diagnostic, setNewDiagnostic, editable}) => {
    ...
    return (
        <div
            ...
            {open ? <DiagnosticModal
                ...
                diagnosticId={diagnostic.diagnosisId}
            ...
            <div className='space-y-2 py-2 w-full group-hover:bg-gray-50 rounded-md'>
                <div
                    ...
                    <p className='text-gray-600 line-clamp-2'>{diagnostic.description}</p>
                </div>
                <div className='group-hover:bg-gray-50 flex justify-between items-center px-3 group'>
                    ...
                    <p className='flex items-center justify-end text-xs font-medium text-gray-600 rounded-full bg-accent border px-2 py-0.5'>
                        <span className='mx-1'>{moment(diagnostic.updatedAt).format('DD-MM-YYYY')}</span>
                        <span className='w-1.5 h-1.5 bg-gray-400 rounded-full'/>
                        <span className='mx-1'>{moment(diagnostic.updatedAt).format('H:mm')}</span>
                    </p>
                </div>
            </div>
        </div>
